-- ---------------------------------------------------------------------------
-- Atlas Knowledge Corpus — Fix industry_ingest_corpus function
--
-- The original function was created before the knowledge tables were fully
-- committed, causing PostgreSQL to fail to resolve table references.
-- This migration drops and recreates the function with explicit
-- schema-qualified references.
--
-- SAFE: DROP FUNCTION does not drop data. Only the function definition is
-- replaced. All tables (atlasIndustryKnowledge, atlasIndustryProvenance,
-- atlasIndustryRelationships) remain intact with their data.
-- ---------------------------------------------------------------------------

-- Drop the broken function (safe — only drops the function, not tables)
DROP FUNCTION IF EXISTS public.industry_ingest_corpus(jsonb, jsonb, jsonb, text);

-- Recreate with fully qualified table references
